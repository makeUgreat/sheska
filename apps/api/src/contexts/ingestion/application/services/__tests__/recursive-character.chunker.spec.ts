...
import {
  ...
  DEFAULT_SEPARATORS,
} from '../recursive-character.chunker';

function buildChunker(chunkSize: number, chunkOverlap: number) {
...
describe('RecursiveCharacterChunker', () => {
  describe('기본 동작', () => {
    it('빈 문자열은 빈 배열을 반환한다', () => {
      const chunker = buildChunker(100, 10);
    ...
    it('index는 0-based 순서로 부여된다', () => {
      const chunker = buildChunker(7, 0);
  ...
  describe('경계 케이스', () => {
    it('text length가 chunkSize와 정확히 같으면 분할하지 않는다', () => {
      const chunker = buildChunker(10, 0);
    ...
    it('chunkOverlap이 0이면 청크 간 중복이 없다', () => {
      const chunker = buildChunker(5, 0);
  ...
  describe('구분자 분할 체인', () => {
    ...
    it('줄바꿈이 없으면 공백 기준으로 분할한다', () => {
      const chunker = buildChunker(6, 0);
    ...
    it('연속된 \\n\\n 사이의 빈 문자열은 청크에 포함되지 않는다', () => {
      ...
      const chunker = buildChunker(12, 0);
    ...
    it('단락들이 chunkSize 내에 들어오면 합쳐서 하나의 청크로 반환한다', () => {
      const chunker = buildChunker(50, 0);
  ...
  describe('오버랩', () => {
    it('문자 단위 분할 시 오버랩 내용이 다음 청크 앞부분에 포함된다', () => {
      const chunker = buildChunker(8, 3);
      const result = chunker.chunk('abcdefghij');
    ...
    it('단락 분할 시 오버랩이 적용되어 앞 청크 내용이 다음 청크에 포함된다', () => {
      const chunker = buildChunker(15, 5);
      const text = 'abcde\n\nfghij\n\nklmno';
  ...
  describe('문자 단위 분할 fallback', () => {
    ...
    it('chunkSize보다 훨씬 긴 단어도 모든 문자를 보존한다', () => {
      const chunker = buildChunker(3, 0);
    ...
    it('chunkOverlap이 chunkSize에 근접해도 무한 루프 없이 종료한다', () => {
      const chunker = buildChunker(5, 4);
