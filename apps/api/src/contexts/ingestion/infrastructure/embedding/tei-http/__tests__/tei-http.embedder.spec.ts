...
import { TeiHttpEmbedder } from '../tei-http.embedder';

describe('TeiHttpEmbedder', () => {
  let client: TeiHttpEmbedder;
  ...
  beforeEach(() => {
    ...
    client = new TeiHttpEmbedder(configService);
  });
  ...
  it('성공 시 임베딩과 모델을 반환한다', async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn().mockResolvedValue({
        ok: true,
        json: () => Promise.resolve([fakeEmbedding]),
      }),
    );
    ...
    expect(fetch).toHaveBeenCalledWith(`${baseUrl}/embed`, {
      method: 'POST',
  ...
  it('TEI가 에러 상태로 응답하면 BAD_RESPONSE InfrastructureException을 던진다', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue({
        ok: false,
