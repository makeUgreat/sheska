...
- 어떤 규칙을 데이터베이스에 둘지는 "동시 쓰기 상황에서 애플리케이션이 스스로 보장할 수 있는가"로 판단한다.
  - 유일성 규칙은 애플리케이션 코드로 보장할 수 없다. 동시에 실행된 두 트랜잭션이 모두 "해당 행이 없다"를 읽고 둘 다 insert한다. 두 번째를 거부할 수 있는 건 데이터베이스뿐이다.
  - 비어 있지 않은 문자열, 숫자 범위, 허용된 상태 값처럼 쓰려는 행만 보고 판단할 수 있는 규칙은 쓰기 전에 이미 도메인 객체가 보장한다. 같은 내용을 `CHECK`로 다시 쓰면 추가로 보장되는 것 없이 규칙을 고칠 곳만 둘로 늘어난다.
  - 아래 두 목록을 가르는 기준이 이것이다. 데이터베이스 제약은 애플리케이션이 막을 수 없는 경쟁 조건을 막되, 도메인이 이미 내린 판단을 반복하지 않는다.
- 비즈니스 불변식이라도 그 불변식 자체가 유일성 규칙이면 unique constraint로 강제할 수 있다.
  - 예: "source 하나당 활성 post 하나"는 비즈니스 규칙이지만, 두 쓰기가 경쟁할 때 이를 지켜주는 건 partial unique index뿐이다.
  - 데이터베이스에서 강제한다고 해서 규칙의 의미가 그쪽으로 옮겨가는 것은 아니다. 규칙의 의미는 여전히 도메인이 소유하고 도메인이 검사한다. 제약이 있는 이유는 그 검사만으로는 경쟁 조건을 이길 수 없기 때문이다.
