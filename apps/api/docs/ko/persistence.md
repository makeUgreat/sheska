...
- Persistence mapper의 restore method는 throw 대신 `Result`를 반환하는 것을 기본으로 한다. Domain restoration이 domain error를 반환하면 mapper와 repository는 그 error를 그대로 통과시켜야 한다.
- Row를 복원하는 중 발생했다는 이유만으로 mapper가 반환한 domain error를 repository error 또는 persistence error로 감싸지 않는다.
