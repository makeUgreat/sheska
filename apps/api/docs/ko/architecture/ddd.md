...
- 컨텍스트 이름은 그 컨텍스트가 담는 모델이나 능력을 가리킨다. 애그리거트 이름이나 테이블 이름을 쓰지 않는다.
  - 애그리거트 이름을 컨텍스트에 붙이면 두 번째 애그리거트가 들어갈 자리가 없어진다. 그 애그리거트는 자기
    컨텍스트를 갖게 되고, 실제로는 같은 모델인데 ACL과 포트와 레이어 세트를 새로 얻는다.
  - 예: `library`(보유한 노트를 동기화하고 발행한다), `ingestion`(본문을 임베딩으로 만든다).
- 애그리거트가 하나 더 생긴다는 사실은 컨텍스트 분리의 근거가 아니다.
  - 애그리거트는 일관성 경계이고 컨텍스트는 언어 경계다. 한 컨텍스트가 여러 애그리거트를 갖는 것이 정상이다.
  - 분리의 근거는 같은 단어가 양쪽에서 다른 뜻이고 넘어올 때 번역이 필요하다는 것이다.
- 컨텍스트를 나눴다면 그 경계가 데이터 접근에서도 성립하는지 확인한다.
  - 한쪽이 다른 쪽 테이블을 직접 읽어야만 동작한다면 그건 두 모델이 아니라 하나다.
  - 읽기 모델의 제한된 예외는 [context integration 컨벤션](./context-integration.md#읽기-모델의-크로스-컨텍스트-sql-예외)을 따른다.
