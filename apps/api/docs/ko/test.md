...
- Value object와 domain value는 valid construction, normalization, invariant violation, boundary value, equality 또는 identity behavior, immutability를 우선적으로 검증한다.
- Aggregate와 entity는 lifecycle creation과 restoration, state transition, consistency boundary 보호, domain event emission, invalid domain action에 대한 domain error를 우선적으로 검증한다.
...
- Integration spec file은 external boundary, context, architecture layer 기준으로 나누는 것을 선호한다. 예를 들어 별도 external boundary directory가 필요 없는 HTTP controller adapter에는 `apps/api/test/contexts/posts/presentation/posts-http.controller.integration-spec.ts`, Postgres 기반 repository adapter에는 `apps/api/test/postgres/contexts/sources/infrastructure/persistence/source.repository.integration-spec.ts`를 사용한다.
