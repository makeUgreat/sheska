...
- Aggregate와 entity는 lifecycle creation과 restoration, state transition, consistency boundary 보호, domain event emission, invalid domain action에 대한 domain failure를 우선적으로 검증한다.
...
- Use case 단위 테스트는 use case가 조율하는 비즈니스 규칙 흐름이 드러나는 case로 작성하는 것을 기본으로 한다. 비즈니스 상황별로 case를 나누고, 입력과 collaborator 결과로 rule branch를 명확히 드러내며, private helper 호출 순서보다 최종 decision 또는 side effect를 검증한다.
...
- Collaborator는 port boundary에서 mock 또는 stub으로 대체한다. Collaborator outcome을 설정해 각 business branch를 명확히 만들고, 최종 result와 관찰 가능한 port interaction을 검증한다.
...
- Integration spec file은 external boundary, context, architecture layer 기준으로 나누는 것을 선호한다. 예를 들어 별도 external boundary directory가 필요 없는 HTTP controller adapter에는 `apps/api/test/contexts/sources/presentation/sources-http.controller.integration-spec.ts`, Postgres 기반 repository adapter에는 `apps/api/test/postgres/contexts/sources/infrastructure/persistence/source.repository.integration-spec.ts`를 사용한다.
...
통합 테스트가 Postgres, Redis, object storage, message broker, 실제 external API처럼 전용 runtime boundary를 필요로 한다면 해당 테스트를 `test/{boundary}/` 아래로 묶는 것을 선호한다.
Boundary directory는 외부 runtime dependency를 나타내고, 그 아래 directory는 bounded context, layer, target을 나타낸다.

File name은 참여하는 모든 구현체가 아니라 test target을 나타내는 데 사용한다.
예를 들어 repository, hashing, queue, database 구현 선택을 file name에 모두 넣기보다 `test/postgres/contexts/sources/application/use-cases/upload-source.use-case.integration-spec.ts`를 선호한다.
Application integration test는 해당 boundary의 대표 production composition을 검증해야 하며, 구현체별 세부 동작은 관련 adapter contract 또는 integration test에 둔다.

Boundary-specific setup과 support file은 `test/postgres/support/`처럼 같은 boundary directory 아래에 둔다.
여러 integration boundary가 공유하는 helper는 `test/support/` 아래에 둔다.
...
단위 테스트는 adapter code 자체가 소유한 동작을 검증해야 한다. 예를 들어 external 또는 persistence shape와 domain object 사이의 mapping, domain restoration failure 보존, adapter 또는 infrastructure failure를 application-owned failure contract로 변환하는 동작, 실제 외부 I/O 없이 증명할 수 있는 adapter-specific branching을 단위 테스트에서 다룬다.
...
같은 observable result를 검증하더라도 책임이 다르면 제한적으로 중복을 허용할 수 있다. 예를 들어 단위 테스트에서 fake database로 검증한 repository failure contract를, 통합 테스트에서는 실제 database constraint가 같은 contract로 이어지는지 확인할 수 있다.
