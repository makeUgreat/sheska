...
Use the GitHub CLI (`gh`) to create PRs. If push succeeds but PR creation is
blocked, report the pushed branch and GitHub compare URL.
...
6. Push and open. Single PR: check `gh pr view` first. If one is already
   ...
   instead (see Stacked PRs) rather than folding it in. Stacked set:
...
7. Report verification gaps and the merge order in the final response.
